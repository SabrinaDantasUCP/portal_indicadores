...
"""
generar_indicadores_encuestas.py
...
Como usar (VPNs separadas — bio e SQL Server não estão acessíveis ao mesmo tempo):
  ...
  2. Desconecte, conecte na VPN do SQL Server/ERP e rode SÓ isso
     (gera respuestas_raw.csv, contacto_raw.csv e encuesta_raw.csv):
       ...
       exportar_respuestas(id_encuesta=3, output_dir="./output")
  ...
  3. Sem precisar de nenhuma VPN, junte os CSVs e gere os indicadores:
       ...
       exports = generar_indicadores(
           id_encuesta=3, anho=2026, semestre=2,
  ...
  Alternativa via terminal, se tiver as duas VPNs ativas ao mesmo tempo:
       python generar_indicadores_encuestas.py --id-encuesta 3 --anho 2026 --semestre 2
"""
...
def fetch_contacto() -> pd.DataFrame:
    log.info("Buscando Sistema.Usuario + dbo.Contacto (IdUsuario -> IdAlumnoExterno)")
    engine = get_mssql_engine()
    with engine.connect() as conn:
        df = pd.read_sql(text(QUERY_CONTACTO), conn)
...
def exportar_respuestas(id_encuesta: int, output_dir: str = None) -> str:
